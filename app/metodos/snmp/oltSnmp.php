...
class nSnmp
{
    ...
    public function __construct($host = '', $comm = '')
    {
        ...
        if ($host !== '' && $host !== null) {
            $profile = $this->db->GetSnmpProfile($host);
        }

        if (!$profile) {
            // No hay perfil válido: dejamos $this->sesion en null.
            // walk()/get()/gett() ya contemplan este caso con ['error'=>'No sesion'].
            error_log("[nSnmp] Host '$host' sin perfil válido en olts_list");
            $this->sesion = null;
            return;
        }
